...
argument-hint: "[task-group: http | grpc | db | pipeline | tuning | all]"
---
...
/trace-matrix grpc
/trace-matrix db
/trace-matrix pipeline
/trace-matrix tuning
/trace-matrix all
```

Without an argument, process all task groups sequentially (may require multiple passes if context is limited).
...
- `internal/report/report.go`: Go PDF pipeline (direct DB queries)
...
Use the Edit tool to make targeted edits. Do not rewrite entire sections unless the structure has changed significantly.
...
2. The field is used in a TeX template or chart
...
- One task group per pass if context is limited. Report progress and wait for the next invocation.
- Preserve existing notes in cells unless tracing contradicts them.
- Every edit must leave MATRIX.md in a valid, consistent state.
