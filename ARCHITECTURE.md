...
L10 clients) with a permanent number-to-concept mapping. The canonical layer
model, the segmented concept status chart with implemented vs planned nodes,
and the per-layer literature alignment all live in
[docs/lidar/architecture/LIDAR_ARCHITECTURE.md](docs/lidar/architecture/LIDAR_ARCHITECTURE.md).
Treat that document as the single source for layer scope, package
boundaries, and stability guarantees.
