...
class TelraamLogger:
    ...
    def get_latest_date_from_db(self) -> str:
        ...
        try:
            ...
            logger.error(f"Could not read latest date from database: {e}. Using configured start date instead.")
            return self.config["start_date"]
    ...
    async def insert_traffic_data(self, traffic_data: List[Dict]) -> int:
        ...
        try:
            ...
            logger.error(f"Could not insert traffic data into database: {e}. Check that align.db is not locked.")
    ...
    async def check_api_authorization(self) -> bool:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{api_url}/v1", headers=headers) as response:
                    if response.status == 200:
                        ...
                        logger.error("API authorisation denied — check your API key in config_telraam.json.")
                        return False
                    else:
                        logger.error(
            ...
            logger.error(f"Could not reach the Telraam API: {e}. Check your network connection.")
            return False
                ...
                try:
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            logger.error("API authentication was rejected. Check your API key.")
                            break
    ...
    async def run(self):
        ...
        try:
            ...
            if not await self.check_api_authorization():
                logger.error("API authorisation check did not pass. Please verify your credentials and try again.")
                return
            ...
            if total_inserted > 0:
                ...
                logger.warning("No records were inserted — the API may have returned empty results for this period.")
...
async def main():
    try:
        ...
        logger.error(f"Could not start the application: {e}. Check config_telraam.json and database access.")
