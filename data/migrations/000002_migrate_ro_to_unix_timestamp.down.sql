-- Rollback: Convert radar_objects timestamps from UNIX epoch back to ISO8601 text
-- WARNING: This conversion may lose precision

CREATE TABLE IF NOT EXISTS radar_objects_old (
    write_timestamp TEXT DEFAULT (STRFTIME('%Y-%m-%dT%H:%M:%f', 'now')),
...
INSERT INTO radar_objects_old (write_timestamp, raw_event)
SELECT STRFTIME('%Y-%m-%dT%H:%M:%f', write_timestamp, 'unixepoch') AS write_timestamp,
       raw_event
FROM radar_objects;

ALTER TABLE radar_objects RENAME TO radar_objects_new;

ALTER TABLE radar_objects_old RENAME TO radar_objects;

DROP TABLE radar_objects_new;
