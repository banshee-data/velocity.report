...
try:
    ...
    print("Missing Python dependencies for plotting:\n  pip install -r tools/requirements.txt")
...
def parse_args():
    p = argparse.ArgumentParser(description='Plot multisweep raw CSV into grid of line charts')
    p.add_argument('--file', '-f', required=True, help='Raw CSV file (expanded per-bucket columns)')
    p.add_argument('--neighbor', '-n', type=int, default=None, help='Filter by neighbor_confirmation_count (optional)')
    p.add_argument('--out', '-o', default='multisweep-plot.png', help='Output PNG filename')
    p.add_argument('--max-cols', type=int, default=6, help='Max number of noise columns to plot (will pick central ones if many)')
    p.add_argument('--max-rows', type=int, default=6, help='Max number of closeness rows to plot (will pick central ones if many)')
    p.add_argument('--show-iterations', action='store_true', help='Plot individual iteration lines (faint) in each subplot')
...
def main():
    ...
    for col in ['noise_relative', 'closeness_multiplier', 'neighbor_confirmation_count', 'iter']:
    ...
    fig, axes = plt.subplots(nrows=nrows, ncols=ncols, figsize=(3*ncols, 2.5*nrows), squeeze=False)
    ...
    for r_idx, clos in enumerate(clos_vals):
        for c_idx, noise in enumerate(noise_vals):
            ax = axes[r_idx][c_idx]
            sel = df[(df['closeness_multiplier'] == clos) & (df['noise_relative'] == noise)]
            if sel.empty:
            ...
            for bi, colname in enumerate(rates_cols):
                ...
                ax.plot(iters, vals, color=colors[bi], alpha=0.9, linewidth=1.5, label=bucket_labels[bi])
    ...
    fig.suptitle(f'Multisweep: neighbor={args.neighbor if args.neighbor is not None else "all"} — lines=acceptance_rates per bucket')
