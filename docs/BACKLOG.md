...
- [#409] Go logging streams unification (LiDAR packages): migrated all `log.Printf` to `diagf`, `fmt.Printf` to `opsf`, created `recorder/debug.go`, wired logging into [cmd/radar/](../cmd/radar): [design doc](plans/go-structured-logging-plan.md)
...
- Version-bump consolidation: pinned `web/package.json`, `public_html/package.json`, and `docs_html/package.json` to `"0.0.0"`; stripped `--web`/`--docs` targets from `set-version.sh`; `set-build-env.js` now derives `PUBLIC_WEB_VERSION` from the Makefile-canonical `PUBLIC_BUILD_VERSION`; CI version-check workflow simplified. `make version-bump` now updates only `Makefile` + `project.pbxproj`: [design doc](plans/version-bump-consolidation-plan.md)
