...
- Agent knowledge architecture — extract shared knowledge into `.github/knowledge/` modules, create `TENETS.md`, condense agent files to role-specific content, add `CLAUDE.md` entry point for dual-tool (Copilot + Claude) use — [design doc](plans/agent-claude-preparedness-review-plan.md) `L`
