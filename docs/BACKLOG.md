...
- LiDAR schema standardisation (migrations 000030 + 000031) — drop dead columns, rename `peak_speed_mps` → `max_speed_mps`, rename `world_frame` → `frame_id`, rename `scene_hash` → `grid_hash`, rename 7 tables into family model — [design doc](plans/lidar-schema-standardisation-plan.md) `L`
