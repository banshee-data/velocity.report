...
- Non-deterministic DBSCAN input subsampling: `uniformSubsample` in `internal/lidar/l4perception/cluster.go` seeds from `time.Now().UnixNano()` mixed with a monotonic counter, and runs whenever a frame exceeds `foreground_max_input_points` (default 8000). Replaying the same PCAP can therefore produce different clusters on busy frames, which undermines the deterministic-replay property that HINT label carry-over, sweep comparison, and the classification corpus all assume. `golden_replay_test.go` runs on synthetic data below the cap and does not catch it. Replace with a content-derived seed and add a replay-equality test above the cap `M` {math}
