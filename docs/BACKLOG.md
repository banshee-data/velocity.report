...
- Version-bump consolidation: pinned `web/package.json`, `public_html/package.json`, and `docs_html/package.json` to `"0.0.0"`; stripped `--web`/`--docs` targets from `set-version.sh`; removed the now-redundant `<meta name="app-web-version">` tag and `PUBLIC_WEB_VERSION` env var from the web build (the surviving `app-build-version` meta tag is sourced from the Makefile-canonical `PUBLIC_BUILD_VERSION`); CI version-check workflow simplified. `make version-bump` now updates only `Makefile` + `project.pbxproj`: [design doc](plans/version-bump-consolidation-plan.md)
