...
- v0.5.0 backward compatibility shim removal — retain `avg_speed_mps` as a running mean alongside `p50_speed_mps` in REST API/track store/DB; sweep legacy fields; deploy executor compat methods — [design doc](plans/v050-backward-compatibility-shim-removal-plan.md) `M`
