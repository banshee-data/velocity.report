...
- [#290] (#11) Finish serial configuration rollout: wire DB-backed serial config into radar startup (`internal/cmd/server/radar.go` still builds the runtime serial config from CLI flags, not the `radar_serial_config` table), add a clear apply/reload path, ship device/baud auto-detect endpoints, and publish the operator guide; the scaffolding shipped in #290 but this completion work did not, and the item was previously mislabelled Complete: [implementation plan](plans/serial-configuration-implementation-plan.md) `M`
...
- [#290] (#11) Serial configuration schema, API, and UI scaffolding: `radar_serial_config` table, serial settings section on `/settings`, and the supporting handlers: [implementation plan](plans/serial-configuration-implementation-plan.md). The rollout-completion work described in the original entry (DB-backed startup wiring, apply/reload path, auto-detect endpoints, operator guide) had **not** shipped and has been returned to v0.5.1
