...
- Context propagation in HTTP handlers — removed 8 `_ = r` placeholders from `internal/api/server.go`; added `context.Context` to 10 DB methods in `site.go`/`site_report.go`; switched to `*Context` SQL variants; added cancellation test — [design doc](plans/go-codebase-structural-hygiene-plan.md)
- `serialmux.CurrentState` race condition — replaced unsynchronised package-global map with `sync.RWMutex`-backed private state; exported `CurrentStateSnapshot()` and `ResetCurrentState()` — [design doc](plans/go-codebase-structural-hygiene-plan.md)
