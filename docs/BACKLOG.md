...
- [#461] localhost listen defaults: default the main and LiDAR HTTP listeners to `127.0.0.1` (operators opt into LAN exposure with `0.0.0.0:PORT`) with aligned docs/tests. The `/command` allowlist was dropped after confirming the OPS24x API command set is non-destructive (no firmware-flash command); access control, not command filtering, is the mitigation `S`
