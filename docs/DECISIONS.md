...
| Metrics/stats consolidation   | 1 (3)           | 2 (4)        | 1 (2)            | 2 (2)       | 3 (3)         | **14** | v0.8      |
...
**Why v0.8 exists (Radar Polish & Automation):** v0.7 initially carried 16 items — too many for a single milestone. v0.8 collects items that either (a) don’t fit the frontend theme (speed limit schedules, cosine error correction), (b) depend on v0.7 completing first (metrics/stats consolidation follow-through, GitHub Releases CI after frontend stabilisation), or (c) are conditional on other work landing (macOS palette constants, LayerChart policy). The split keeps v0.7 focused on the Svelte migration at 10 items, while v0.8 provides a natural bridge to v1.0 production readiness. Items scoring above the v0.7 threshold (≥ 14) may still appear in v0.8 when they don’t fit the milestone theme or depend on v0.7 deliverables — the matrix scores placement eligibility, not obligation.
...
v0.8 (Radar Polish & Automation)
  ...
  └── Metrics/stats consolidation follow-through (C/D)
        │
