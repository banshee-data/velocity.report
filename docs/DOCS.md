...
- `/docs/tools/` # Documentation-asset generators (not production code)
...
- [docs/tools/](tools) contains generators for documentation assets, each in its own directory with a README and its committed output under `generated/`. These are documentation tooling, not production code, and must not be imported by the Go server, the web frontend, or the visualiser.
