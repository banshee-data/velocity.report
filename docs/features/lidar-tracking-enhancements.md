# LIDAR Tracking Enhancements: Foreground Extraction & Classification Evolution

## Executive Summary

This document outlines a comprehensive plan to enhance the LIDAR tracking system's foreground point cloud extraction, classification capabilities, and introspection/charting for track quality assessment. The plan addresses the current state of tracking, identifies opportunities for ML-ready feature extraction, and proposes phased enhancements to improve track quality, coverage analysis, and noise reduction.
...
- ⚠️ Limited introspection/metrics for track quality
- ⚠️ No ML model support for classification
- ⚠️ Minimal coverage metrics for noise/unknown classification
...
### **Phase 2: ML-Ready Classification Infrastructure**

**Goal:** Enable transition from rule-based to ML-based classification with feature extraction.

#### 2.1 Feature Vector Extraction

**New Module:** `internal/lidar/features.go`

**Feature Categories:**

**Spatial Features (Geometry):**
```go
...
// Rule-based classifier (existing)
type RuleBasedClassifier struct { ... }

// Static ML classifier (new)
type StaticMLClassifier struct {
    ModelPath   string
    ModelType   string // "random_forest", "svm", "xgboost"
...
**Model Loading:**
```go
func LoadStaticClassifier(modelPath string) (Classifier, error) {
    // Support formats:
    // - ONNX (cross-platform, recommended)
    // - scikit-learn pickle (Python dependency)
    // - XGBoost binary (native Go binding)
    // - TensorFlow Lite (embedded models)
}
```

**Feature Engineering Pipeline:**
1. Extract raw features from `TrackedObject`
2. Compute derived features (aspect ratios, straightness, etc.)
3. Apply feature scaling (z-score normalization)
4. Pass to model for inference
5. Post-process: apply confidence thresholds, handle "unknown" class

**Training Data Export:**
- Export `RunTrack` features to CSV/JSON for offline training
- Include user labels from `lidar_run_tracks.user_label`
- Store feature version metadata for reproducibility

#### 2.3 Model Comparison Framework

**Goal:** A/B test rule-based vs ML classifiers.

**API Endpoint:**
```
POST /api/lidar/classify/compare
{
  "run_id": "run_20241210_001",
  "models": ["rule_based_v1", "random_forest_v1", "xgboost_v1"]
}

Response:
{
  "comparison": {
    "rule_based_v1": { "accuracy": 0.72, "f1_scores": {...}, "confusion_matrix": [...] },
    "random_forest_v1": { "accuracy": 0.85, "f1_scores": {...}, "confusion_matrix": [...] },
    "xgboost_v1": { "accuracy": 0.88, "f1_scores": {...}, "confusion_matrix": [...] }
  },
...
- Confusion matrix (predicted vs user labels)
- Confidence calibration (histogram of confidences vs accuracy)
