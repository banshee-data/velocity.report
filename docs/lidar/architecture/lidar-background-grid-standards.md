...
| **VTK/ParaView** grids (LidarView)          | `vtkImageData` / `vtkStructuredGrid` or `vtkPolyData` | LidarView/ParaView favour VTK XML (`.vti/.vts/.vtp`) for interchange and visualisation. | Standardized I/O; strong tooling (ParaView, VTK.js); preserves attributes.                       | Format standardizes containers, not background semantics; still requires our own field schema.                                                   |
...
2. **Add an export shim** that converts the grid to a standard container for tooling, without changing the core model:
   - For visualisation: emit **VTK `vtkImageData` or `vtkStructuredGrid`** (cell fields: mean range, spread, times seen, frozen-until) and optionally **PCD** with per-cell attributes for LidarView/ParaView inspection.
