...
- **Target:** v0.5.1 (single primary binary cutover, static-binary Tailscale, embedded Typst, Typst source archive, read-only SQL path), v0.5.2 (final apt purge + stage simplification); deliberately ahead of v0.6.0 wide release so the public install path is "one primary binary, one image, one update command" before we hit a wider audience.
