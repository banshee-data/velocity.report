...
- **Status:** Complete — implemented in v0.5.1. Dispatcher (`cmd/velocity`), namespaces, versioned `/opt/velocity-report/versions/<v>/` layout with `current`/`previous` symlinks, `renameat2` atomic swap, retention pruning, `GET /api/version`, single `velocity-<v>-<os>-arm64` release artifact, and the tightened sudoers all landed. The transitional `velocity-ctl` shim has since been removed (it never shipped in a release). Folded as work unit A of the consolidation plan; shipped model documented in [distribution-packaging.md](../platform/operations/distribution-packaging.md).
