...
- Add `--worker` flag to [internal/cmd/server/radar.go](../../internal/cmd/server/radar.go)
...
- No separate binary: `--worker` is a flag on the existing [internal/cmd/server/radar.go](../../internal/cmd/server/radar.go) entry point, consistent with `--enable-lidar`, `--disable-radar`, `--debug`
...
```
internal/cmd/server/radar.go                          (--worker flag, mode branching)
