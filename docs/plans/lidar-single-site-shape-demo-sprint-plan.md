...
roadmap. Its body-extent and identity evidence feeds partial-view bumper estimation, v0.5.2
occlusion continuity and v0.5.4 trail validation. Add held-out visible-face changes and bounded
occlusion/reacquisition cases where the recording supports them. Do not claim unseen bumper truth
from membership masks. Proposed next-branch consolidation makes this demo a milestone of the
annotation plan; no files or historical acceptance records are merged here.
The v0.5.2 critical path ends with reviewed solid-body and trail evidence. Compact descriptor
scoring and its inspection panel are v0.5.6 follow-through; they must not delay the headway oracle,
...
`6252be7f2`, provides part of Day 1's backend. Revision rejection, recoverable history, and
restore-as-new-revision are now implemented; selection UI, JSON model fitting, seeded shape
