...
> **Date:** 2026-02-11
> **Relates to:** [distribution-packaging-plan.md](./distribution-packaging-plan.md) § 8.2
...
`.img` file that can be flashed by *any* tool (rpi-imager, balenaEtcher, `dd`).
...
|-----------|--------|-------------|
| `velocity-report` (Go server) | Cross-compiled ARM64 binary | `/usr/local/bin/velocity-report` |
...
│   ├── 00-packages                 # APT packages (texlive, sqlite3, etc.)
│   ├── 01-velocity-binaries/
│   │   ├── 00-run.sh              # Copy pre-built Go binaries
│   │   └── files/
│   │       ├── velocity-report     # ARM64 binary (from CI artifact)
  ...
  build-binaries:
    ...
    steps:
      ...
      - name: Cross-compile Go binaries (ARM64)
        run: make build-radar-linux build-deploy-linux
...
| TeX Live (xetex + fonts) | ~800 MB |
| Python 3 + venv + PDF deps | ~200 MB |
| Go binaries (server + deploy) | ~30 MB |
| Web frontend (static) | ~5 MB |
| System config + udev rules | < 1 MB |
| **Total (uncompressed)** | **~1.5 GB** |
| **Total (xz compressed)** | **~600–900 MB** |

> **Note:** TeX Live is the dominant size contributor.  A future optimisation
> could use a minimal TeX Live installation or pre-compile LaTeX templates to
...
| Criterion | Monorepo (`velocity.report/imager/`) | Separate Repo (`velocity-imager`) |
|-----------|--------------------------------------|----------------------------------|
...
**Use a separate repository** (`banshee-data/velocity-imager` or similar).
...
|------|----------|------------|
| **TeX Live size bloat** — full installation is 4+ GB | High | Use `texlive-xetex` + specific font packages only; consider `tinytex` or pre-compiled templates |
...
| **First-boot configuration** — users need to set Wi-Fi before the device has a screen | Medium | Leverage rpi-imager's built-in Wi-Fi/SSH customisation; or provide a `velocity-report.conf` on the boot partition |
...
- [ ] Verify `make build-radar-linux` produces a working ARM64 binary
...
- [ ] Fork `raspberrypi/rpi-imager` to `banshee-data/velocity-imager`
...
```json
{
  ...
  "os_list": [
    {
      "name": "velocity.report (Recommended)",
      "description": "Privacy-first traffic monitoring — full stack with radar support, PDF reporting, and web dashboard. Based on Raspberry Pi OS Lite (Bookworm, 64-bit).",
      "url": "https://github.com/banshee-data/velocity.report/releases/download/v1.0.0/velocity-report-v1.0.0.img.xz",
      "extract_size": 1610612736,
      "extract_sha256": "<sha256-of-uncompressed-img>",
      "image_download_size": 838860800,
    ...
    {
      "name": "velocity.report (Minimal — No LaTeX)",
      "description": "Lightweight build without PDF generation. Radar monitoring and web dashboard only. ~600 MB smaller.",
      "url": "https://github.com/banshee-data/velocity.report/releases/download/v1.0.0/velocity-report-minimal-v1.0.0.img.xz",
      "extract_size": 943718400,
      "extract_sha256": "<sha256-of-uncompressed-img>",
...
- The os-list JSON is fetched by rpi-imager, but this only reveals that someone
  is *looking at* the velocity.report catalogue, not using it

---

## 12. Open Questions

1. **Minimum supported Raspberry Pi model?** — Pi 4 only, or also Pi 3B+ (which
   has less RAM and a 32-bit option)?  The Go binary and TeX Live both benefit
   from 4 GB RAM.

2. **Image variant strategy** — Should we ship one image or two (full vs.
   minimal/no-LaTeX)?  The LaTeX dependency adds ~800 MB.

3. **Auto-update mechanism** — Should the image check for velocity.report binary
   updates on boot?  This conflicts with the privacy-first principle (makes
   network requests).  Alternative: manual update via `velocity-deploy upgrade`.

4. **Wi-Fi country code** — The rpi-imager first-boot flow sets Wi-Fi country.
   If users skip this, wireless may not work.  Should we add a fallback?

5. **LIDAR support** — The current image spec focuses on radar.  Should LIDAR
   packages (network config for 192.168.100.x) be included in the base image
   or as a separate variant?

6. **Naming** — `velocity-imager` or `velocity-report-imager` for the separate
   repository?
...
- [velocity.report distribution-packaging-plan.md](./distribution-packaging-plan.md) § 8.2
...
| **Repository for imager fork** | **Separate repo** (`banshee-data/velocity-imager`) | Different tech stack (C++/Qt), release cadence, and contributor profile |
| **Image build scripts** | **Monorepo** (`velocity.report/image/`) | Tightly coupled to server releases; same CI pipeline |
| **Image variants** | Full (with LaTeX) + Minimal (without) | Respect bandwidth constraints; not everyone needs PDF generation |
