...
**Scope:** reserve percentiles for grouped/report aggregates only, back out per-track percentile proto/API work, rename raw `peak` to `max`, define replacement track-level speed metrics, and keep one canonical aggregate percentile path
...
That overlap is the design error. A track-level `p50` or `p85` sounds like the
same thing as a report-level `p50` or `p85`, but it is not. The public model
...
| -------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| `p50`          | Radar stats API, PDF/reporting, web charts                                     | Present today in LiDAR tracking/storage/API/proto work       | None                                                                     | Keep only for grouped/report aggregates                                |
| `p85`          | Radar stats API, PDF/reporting, web charts                                     | Present today in LiDAR storage/classification/proto work     | None                                                                     | Keep only for grouped/report aggregates                                |
...
| `p98`          | Current high-speed percentile in radar stats, PDF/reporting, and planning docs | Present today in LiDAR storage/classification/proto work     | Replaced earlier `p95` usage                                             | Keep as the high-end aggregate percentile if the repo keeps one        |
...
| --------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `internal/lidar/l6objects` + `internal/lidar/storage/sqlite/track_store.go` | Per-track `p50/p85/p98` are computed from track speed history and persisted today.                                    | Design debt to remove from the canonical track model |
| Visualiser proto/adapter                                                    | Branch-local work exposes per-track `p50/p85/p98` to gRPC/Swift clients.                                              | Must be backed out before merge                      |
| `internal/lidar/monitor/track_api.go` per-track REST                        | Individual track JSON already exposes `p50_speed_mps`.                                                                | Must be backed out, not expanded                     |
| `internal/lidar/monitor/track_api.go` summary REST                          | Summary payloads contain a `p50_speed_mps` placeholder, but grouped percentile semantics are still incomplete.        | Needs redesign around aggregate-only percentiles     |
...
| Planning/docs surface                                                       | Several active plans still say per-track `p50/p85/p98` should be shipped in proto, REST, and UI.                      | Must be rewritten in this PR                         |
...
4. **Track-level speed summaries must use different terminology and different measures.**
   A single track should not expose `p50/p85/p98`; it needs its own metrics such
   as a robust typical speed and a reliable peak speed.
5. **The unmerged per-track percentile proto/API work should be backed out, not expanded.**
...
| --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Names for replacement track metrics                       | Tracks still need public speed summaries, but `p50/p85/p98` can no longer be those names.                                                     | Use working names such as `typical_observed_speed_mps` and `reliable_peak_speed_mps` until the final naming review is done, then record the final ids/aliases in the [metrics registry and observability plan](metrics-registry-and-observability-plan.md). |
...
| Migration strategy for existing internal pXX calculations | Some internal code still computes/stores per-track `p50/p85/p98` today.                                                                       | Stop public exposure first, keep any temporary internal use isolated, and remove the internal percentile calculations once the new track metrics land.                                                                                                      |
...
- [x] Mark earlier proto/API expansion plans for per-track `p50/p85/p98` as superseded.

### Phase 1 - Back Out The Wrong Public Contract

- [ ] Remove branch-local `p50/p85/p98` additions from the `Track` proto, generated bindings, visualiser model/UI, and any new REST contract work tied to them.
...
- [ ] Update tests and docs so `p50/p85/p98` appear only on grouped/report surfaces.
...
- [ ] Replace branch-local/internal dependencies on per-track percentile outputs with the new track metrics or other internal descriptors.
- [ ] Stop treating track-level `p50/p85/p98` as canonical stored fields in new APIs and schemas.
- [ ] Migrate public/raw `peak_speed_mps` references to `max_speed_mps` so "peak" is available for the future filtered metric.
- [ ] Plan migration for historical storage surfaces (`lidar_tracks`, analysis runs, tests) that still carry per-track percentile columns today.
...
- No track-level public field, proto property, or UI label uses `p50/p85/p98`.
