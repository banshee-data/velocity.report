...
| pcap-analyse logic | [cmd/tools/pcap-analyse/main.go](../../cmd/tools/pcap-analyse/main.go)                                      | ~1.6k lines, `package main`, `//go:build pcap`, parses the global `flag.CommandLine`              |

### Gap analysis

1. **Logic is trapped in `package main`.** The analysis pipeline, `Config`, `CaptureStats`, exporters, and benchmark mode live in `cmd/tools/pcap-analyse/main.go` and cannot be imported. They must move to an importable package.
...
1. **Extract the engine** → new `internal/lidar/pcapanalyse/` (importable, `//go:build pcap`). Move the pipeline driver, `Config`, `AnalysisResult`/`CaptureStats`, exporters, and benchmark code from `cmd/tools/pcap-analyse/main.go`. Expose a small entry such as `func Run(cfg Config) (*AnalysisResult, error)` (and the benchmark variant). Keep the existing `--motion` behaviour (which already calls `pcapsplit.BuildTimeline`).
...
5. **Standalone tool stays thin.** Rewrite [cmd/tools/pcap-analyse/main.go](../../cmd/tools/pcap-analyse/main.go) to `func main() { os.Exit(lidar.AnalyseMain(os.Args[1:])) }` (export `AnalyseMain`) so the existing binary, `run-pcap-stats` Makefile targets, and the benchmark CI keep working against the same code path. No behaviour change.
...
| `cmd/tools/pcap-analyse/main.go`                        | Reduce to a thin wrapper over the shared applet   |
...
3. **Retire the standalone binaries?** Keep `cmd/tools/pcap-analyse` as a thin wrapper for now (CI/baseline scripts depend on it); revisit removal once callers migrate to `velocity lidar …`.
