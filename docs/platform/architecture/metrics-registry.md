...
| `interaction.following_valid_time_s`                           | `event_json.measurements.<id>`; its denominator is the sum of `window_json.duration_nanos` where `basis` is `observed` and `kind` is `valid_following` |
