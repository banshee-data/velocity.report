...
//
// Thread-safety:
//   - CurrentMux(), SendCommand(), and Initialise() safely read/delegate to the
...
//     to SendCommand() and Initialise() will return an error, and Subscribe() will
...
//
// This loop runs until Close() is called (signalled via eventFanoutCh being closed).
...
// operations (SendCommand, Initialise, Subscribe, etc.) are in progress.
//
// After Close() is called:
//   - SendCommand() and Initialise() will return an error
...
// Initialise delegates to the active mux. Returns an error if the mux is
// unavailable or has been closed.
func (m *SerialPortManager) Initialise() error {
	m.mu.RLock()
	mux := m.current
	closed := m.closed
	m.mu.RUnlock()
...
func (m *SerialPortManager) ReloadConfig(ctx context.Context) (*SerialReloadResult, error) {
	...
	optionsEqual, err := currentSnap.Options.Equal(normalised)
	if err != nil {
	...
	if samePort {
		...
		newMux, err = m.factory(cfg.PortPath, normalised)
		if err != nil {
			return nil, fmt.Errorf("failed to open serial port %s: %w", cfg.PortPath, err)
		}

		if err := newMux.Initialise(); err != nil {
			newMux.Close()
