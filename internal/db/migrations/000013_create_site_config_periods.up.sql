...
CREATE INDEX IF NOT EXISTS idx_site_config_periods_active ON site_config_periods (site_id, is_active)
    WHERE is_active = 1;

-- Ensure only one active period per site
CREATE TRIGGER IF NOT EXISTS ensure_single_active_period_insert BEFORE INSERT ON site_config_periods WHEN NEW.is_active = 1 BEGIN
...
CREATE TRIGGER IF NOT EXISTS ensure_single_active_period_update BEFORE
