-- lidar_tracks: restore percentile columns
    ALTER TABLE lidar_tracks
      ADD COLUMN p50_speed_mps REAL;

    ALTER TABLE lidar_tracks
      ADD COLUMN p85_speed_mps REAL;

    ALTER TABLE lidar_tracks
      ADD COLUMN p95_speed_mps REAL;

-- lidar_tracks: restore peak name
    ALTER TABLE lidar_tracks
   RENAME COLUMN max_speed_mps TO peak_speed_mps;
...
-- restore world_frame
   ...
   RENAME COLUMN frame_id TO world_frame;

    ALTER TABLE lidar_tracks
   RENAME COLUMN frame_id TO world_frame;

    ALTER TABLE lidar_track_obs
   RENAME COLUMN frame_id TO world_frame;

-- restore scene_hash
    ALTER TABLE lidar_bg_regions
   RENAME COLUMN grid_hash TO scene_hash;
