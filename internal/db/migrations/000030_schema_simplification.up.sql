-- lidar_tracks: drop dead percentile columns
    ALTER TABLE lidar_tracks
     DROP COLUMN p50_speed_mps;

    ALTER TABLE lidar_tracks
     DROP COLUMN p85_speed_mps;

    ALTER TABLE lidar_tracks
     DROP COLUMN p95_speed_mps;

-- lidar_tracks: rename peak → max
    ALTER TABLE lidar_tracks
   RENAME COLUMN peak_speed_mps TO max_speed_mps;
...
-- rename world_frame → frame_id on three tables
   ...
   RENAME COLUMN world_frame TO frame_id;
...
-- rename scene_hash → grid_hash on lidar_bg_regions
   ...
   RENAME COLUMN scene_hash TO grid_hash;
