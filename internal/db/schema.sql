...
CREATE INDEX idx_lidar_tracks_quality ON lidar_tracks (track_length_meters, occlusion_count);
   ...
   CREATE TABLE lidar_algorithm_runs (
          run_id TEXT PRIMARY KEY
        , start_unix_nanos INTEGER NOT NULL
        , end_unix_nanos INTEGER
...
CREATE INDEX idx_algorithm_runs_start ON lidar_algorithm_runs (start_unix_nanos);

   CREATE TABLE lidar_algorithm_frame_results (
          run_id TEXT NOT NULL
...
CREATE INDEX idx_frame_results_time ON lidar_algorithm_frame_results (frame_unix_nanos);
