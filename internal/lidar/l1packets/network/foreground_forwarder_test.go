...
func TestForegroundForwarder_ForwardForeground_BufferFull(t *testing.T) {
	...
	points := []l2frames.PointPolar{{Channel: 1, Distance: 10.0}}
	for i := 0; i < 100; i++ {
		ff.ForwardForeground(points)
	}
...
func TestEncodePointsAsPackets_Empty(t *testing.T) {
	...
	packets, err = ff.encodePointsAsPackets([]l2frames.PointPolar{})
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	if packets != nil {
...
func TestBuildPacket_AllBlocks(t *testing.T) {
	...
	var points []l2frames.PointPolar
	for blockID := 0; blockID < 10; blockID++ {
		points = append(points, l2frames.PointPolar{
			Channel:         1,
...
func BenchmarkEncodePointsAsPackets(b *testing.B) {
	...
	now := time.Now().UnixNano()
	points := make([]l2frames.PointPolar, 400) // Typical foreground frame size
...
func BenchmarkBuildPacket(b *testing.B) {
	...
	now := time.Now().UnixNano()
	points := make([]l2frames.PointPolar, 40)
	for i := 0; i < len(points); i++ {
		points[i] = l2frames.PointPolar{
			Channel:   i + 1,
