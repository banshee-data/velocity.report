...
func (m *MockParser) ParsePacket(packet []byte) ([]l2frames.PointPolar, error) {
	m.parseCalled++
	if m.parseErr != nil {
...
func (m *MockFrameBuilder) AddPointsPolar(points []l2frames.PointPolar) {
	m.addCalled++
...
func TestNewUDPListener_WithParser(t *testing.T) {
	parser := &MockParser{
		points:     []l2frames.PointPolar{{Distance: 10.0}},
		motorSpeed: 600,
	}
...
func TestForegroundForwarder_ForwardForeground_Empty(t *testing.T) {
	...
	ff.ForwardForeground([]l2frames.PointPolar{})
}
...
func TestUDPListener_HandlePacket_WithParser(t *testing.T) {
	stats := &MockFullPacketStats{}
	parser := &MockParser{
		points:     []l2frames.PointPolar{{Distance: 10.0}, {Distance: 20.0}},
		motorSpeed: 600,
	}
...
func TestUDPListener_HandlePacket_EmptyPoints(t *testing.T) {
	stats := &MockFullPacketStats{}
	parser := &MockParser{
		points:     []l2frames.PointPolar{}, // Empty points
		motorSpeed: 600,
	}
	frameBuilder := &MockFrameBuilder{}
...
func (m *LegacyFrameBuilder) AddPointsPolar(points []l2frames.PointPolar) {
	m.addCalled++
	m.points = append(m.points, points...)
}
