...
// SetLogWriters configures the three logging streams for the l4perception package.
// Pass nil for any writer to disable that stream.
func SetLogWriters(ops, diag, trace io.Writer) {
	opsLogger = logutil.NewTaggedLogger("[l4perception] ", ops)
	diagLogger = logutil.NewTaggedLogger("[l4perception] ", diag)
	traceLogger = logutil.NewTaggedLogger("[l4perception] ", trace)
}
