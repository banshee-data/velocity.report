...
// TestReadIndexedFramePayload_InvalidFrameLength exercises the
// "invalid frame length" branch in readIndexedFramePayload by writing a
// chunk file whose length prefix claims more bytes than the file contains.
func TestReadIndexedFramePayload_InvalidFrameLength(t *testing.T) {
	dir := t.TempDir()
	basePath := filepath.Join(dir, "test-log")
	framesDir := filepath.Join(basePath, "frames")
	if err := os.MkdirAll(framesDir, 0o755); err != nil {
		t.Fatal(err)
	}

	// Write a minimal chunk: 4-byte length prefix claiming 9999 bytes, but
	// only 4 extra bytes of actual data.
	chunkPath := filepath.Join(framesDir, "chunk_0000.pb")
	buf := make([]byte, 8)
	binary.LittleEndian.PutUint32(buf[0:4], 9999) // frameLen = 9999
	binary.LittleEndian.PutUint32(buf[4:8], 0)    // dummy payload

	if err := os.WriteFile(chunkPath, buf, 0o644); err != nil {
		t.Fatal(err)
	}

	entry := IndexEntry{ChunkID: 0, Offset: 0}
	_, err := readIndexedFramePayload(basePath, entry)
	if err == nil {
		t.Fatal("expected error for invalid frame length")
	}
}

// TestReadIndexedFramePayload_ChunkTooLarge exercises the "chunk file too
// large" branch in readIndexedFramePayload.
func TestReadIndexedFramePayload_ChunkTooLarge(t *testing.T) {
	dir := t.TempDir()
	basePath := filepath.Join(dir, "test-log")
	framesDir := filepath.Join(basePath, "frames")
	if err := os.MkdirAll(framesDir, 0o755); err != nil {
	...
	chunkPath := filepath.Join(framesDir, "chunk_0000.pb")
	// Create a sparse file that appears larger than maxChunkSize.
	f, err := os.Create(chunkPath)
	if err != nil {
		t.Fatal(err)
	}
	// maxChunkSize is 256 MB; truncate past it.
	if err := f.Truncate(maxChunkSize + 1); err != nil {
		f.Close()
