...
// SetLogWriters configures the three logging streams for the recorder package.
// Pass nil for any writer to disable that stream.
func SetLogWriters(ops, diag, trace io.Writer) {
	opsLogger = logutil.NewTaggedLogger("[recorder] ", ops)
	diagLogger = logutil.NewTaggedLogger("[recorder] ", diag)
	traceLogger = logutil.NewTaggedLogger("[recorder] ", trace)
}
