...
func serializeFrameProto(frame *l9endpoints.FrameBundle) ([]byte, error) {
...
func deserializeFrameProto(data []byte) (*l9endpoints.FrameBundle, error) {
...
func frameBundleToStorageProto(frame *l9endpoints.FrameBundle) *pb.FrameBundle {
...
func protoToFrameBundle(pbFrame *pb.FrameBundle) *l9endpoints.FrameBundle {
	...
	if pbFrame.CoordinateFrame != nil {
		frame.CoordinateFrame = l9endpoints.CoordinateFrameInfo{
	...
	if pbFrame.PointCloud != nil {
		...
		frame.PointCloud = &l9endpoints.PointCloudFrame{
			FrameID:         pc.FrameId,
			...
			DecimationMode:  l9endpoints.DecimationMode(pc.DecimationMode),
			DecimationRatio: pc.DecimationRatio,
	...
	if pbFrame.Clusters != nil {
		cs := pbFrame.Clusters
		clusters := make([]l9endpoints.Cluster, len(cs.Clusters))
		for i, c := range cs.Clusters {
			clusters[i] = l9endpoints.Cluster{
				ClusterID:      c.ClusterId,
				SensorID:       c.SensorId,
		...
		frame.Clusters = &l9endpoints.ClusterSet{
			FrameID:        cs.FrameId,
			TimestampNanos: cs.TimestampNs,
			Clusters:       clusters,
			Method:         l9endpoints.ClusteringMethod(cs.Method),
		}
	}

	if pbFrame.Tracks != nil {
		ts := pbFrame.Tracks
		tracks := make([]l9endpoints.Track, len(ts.Tracks))
		for i, t := range ts.Tracks {
			tracks[i] = l9endpoints.Track{
				TrackID:           t.TrackId,
				SensorID:          t.SensorId,
				State:             l9endpoints.TrackState(t.State),
				Hits:              int(t.Hits),
				Misses:            int(t.Misses),
				...
				Confidence:        t.Confidence,
				OcclusionState:    l9endpoints.OcclusionState(t.OcclusionState),
				MotionModel:       l9endpoints.MotionModel(t.MotionModel),
				Alpha:             t.Alpha,
				HeadingSource:     int(t.HeadingSource),
			}
		}
		trails := make([]l9endpoints.TrackTrail, len(ts.Trails))
		for i, trail := range ts.Trails {
			points := make([]l9endpoints.TrackPoint, len(trail.Points))
			for j, p := range trail.Points {
				points[j] = l9endpoints.TrackPoint{
					X:              p.X,
					Y:              p.Y,
					TimestampNanos: p.TimestampNs,
				}
			}
			trails[i] = l9endpoints.TrackTrail{
				TrackID: trail.TrackId,
				Points:  points,
			}
		}
	...
	if pbFrame.Background != nil {
		...
		frame.Background = &l9endpoints.BackgroundSnapshot{
			SequenceNumber: bg.SequenceNumber,
			TimestampNanos: bg.TimestampNanos,
			X:              bg.X,
			Y:              bg.Y,
		...
		if bg.GridMetadata != nil {
			frame.Background.GridMetadata = l9endpoints.GridMetadata{
