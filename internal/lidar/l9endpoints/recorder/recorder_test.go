...
func testFrameBundle(frameID uint64, timestampNanos int64) *l9endpoints.FrameBundle {
	return &l9endpoints.FrameBundle{
		FrameID:        frameID,
...
func writeSingleFrameLog(t *testing.T, basePath string, frame *l9endpoints.FrameBundle, payload []byte) {
	t.Helper()
...
func TestRecordAndReplayRoundTrip(t *testing.T) {
	...
	baseTime := int64(1000000000000)
	originalFrames := make([]*l9endpoints.FrameBundle, 10)
	for i := 0; i < 10; i++ {
...
func TestRecorder_BackgroundFramesExcludedFromHeaderRange(t *testing.T) {
	...
	bgFrame := func(id uint64, ts int64) *l9endpoints.FrameBundle {
		f := testFrameBundle(id, ts)
		f.FrameType = l9endpoints.FrameTypeBackground
		return f
	}
	fgFrame := func(id uint64, ts int64) *l9endpoints.FrameBundle {
		f := testFrameBundle(id, ts)
		f.FrameType = l9endpoints.FrameTypeForeground
		return f
	}

	for _, frame := range []*l9endpoints.FrameBundle{
		bgFrame(1, wallTs),
		fgFrame(2, pcapTs),
...
func TestEmptyFrameRoundTrip(t *testing.T) {
	...
	emptyFrame := func(id uint64, ts int64) *l9endpoints.FrameBundle {
		return &l9endpoints.FrameBundle{
			FrameID:        id,
			TimestampNanos: ts,
			SensorID:       "test-deterministic",
			FrameType:      l9endpoints.FrameTypeEmpty,
			CoordinateFrame: l9endpoints.CoordinateFrameInfo{
				FrameID:        "test/sensor-01",
				ReferenceFrame: "ENU",
			},
	...
	recorded := make([]*l9endpoints.FrameBundle, len(specs))
	var ts int64
	for i, spec := range specs {
		...
		if spec.empty {
			...
			recorded[i].FrameType = l9endpoints.FrameTypeForeground
		}
...
func TestProtoRoundTrip_MaxSpeedMps(t *testing.T) {
	...
	frame.Tracks = &l9endpoints.TrackSet{
		...
		Tracks: []l9endpoints.Track{
			{
				TrackID: "trk-speed-1", MaxSpeedMps: 12.75, SpeedMps: 5.5,
			},
		},
	}
