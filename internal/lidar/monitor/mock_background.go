...
type BackgroundManagerInterface interface {
	...
	GetGridCells() []lidar.ExportedCell
	// GetGridHeatmap returns aggregated heatmap buckets
	GetGridHeatmap(azimuthBucketDeg float64, settledThreshold uint32) *lidar.GridHeatmap
	// GetParams returns the current background parameters
	GetParams() lidar.BackgroundParams
	// SetParams updates background parameters
	SetParams(p lidar.BackgroundParams) error
...
// backgroundManagerWrapper wraps *lidar.BackgroundManager to implement the interface.
type backgroundManagerWrapper struct {
	bm *lidar.BackgroundManager
}

func (w *backgroundManagerWrapper) GetGridCells() []lidar.ExportedCell {
	return w.bm.GetGridCells()
}

func (w *backgroundManagerWrapper) GetGridHeatmap(azimuthBucketDeg float64, settledThreshold uint32) *lidar.GridHeatmap {
	return w.bm.GetGridHeatmap(azimuthBucketDeg, settledThreshold)
}

func (w *backgroundManagerWrapper) GetParams() lidar.BackgroundParams {
	return w.bm.GetParams()
}

func (w *backgroundManagerWrapper) SetParams(p lidar.BackgroundParams) error {
...
func (w *backgroundManagerWrapper) GetGrid() *lidar.BackgroundGrid {
	return w.bm.Grid
}
...
type MockBackgroundManager struct {
	...
	SetParamsErr   error
	SetNoiseErr    error
	SetParamsCalls []lidar.BackgroundParams
	SetNoiseCalls  []float32
}

// GetGridCells returns the configured cells.
func (m *MockBackgroundManager) GetGridCells() []lidar.ExportedCell {
	return m.Cells
}

// GetGridHeatmap returns the configured heatmap.
func (m *MockBackgroundManager) GetGridHeatmap(azimuthBucketDeg float64, settledThreshold uint32) *lidar.GridHeatmap {
	return m.Heatmap
}

// GetParams returns the configured params.
func (m *MockBackgroundManager) GetParams() lidar.BackgroundParams {
	return m.Params
}

// SetParams records the call and returns configured error.
func (m *MockBackgroundManager) SetParams(p lidar.BackgroundParams) error {
...
func (m *MockBackgroundManager) GetGrid() *lidar.BackgroundGrid {
	return m.Grid
}
