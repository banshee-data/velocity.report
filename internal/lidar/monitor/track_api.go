...
// TrackerConfigRequest represents the JSON body for setting tracker configuration.
// All fields are optional pointers — only non-nil values are applied.
type TrackerConfigRequest struct {
...
// TrackerConfigResponse represents the current tracker configuration.
type TrackerConfigResponse struct {
	...
	OcclusionCovInflation float32 `json:"occlusion_cov_inflation"`
	HitsToConfirm         int     `json:"hits_to_confirm"`
	MaxMisses             int     `json:"max_misses"`
	MaxMissesConfirmed    int     `json:"max_misses_confirmed"`
...
func (api *TrackAPI) handleTrackerConfig(w http.ResponseWriter, r *http.Request) {
	if api.tracker == nil {
		api.writeJSONError(w, http.StatusServiceUnavailable, "in-memory tracker not available")
		return
	}
	...
	switch r.Method {
	case http.MethodGet:
		cfg := api.tracker.Config
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(TrackerConfigResponse{
			GatingDistanceSquared: cfg.GatingDistanceSquared,
			ProcessNoisePos:       cfg.ProcessNoisePos,
			ProcessNoiseVel:       cfg.ProcessNoiseVel,
			MeasurementNoise:      cfg.MeasurementNoise,
			OcclusionCovInflation: cfg.OcclusionCovInflation,
			HitsToConfirm:         cfg.HitsToConfirm,
			MaxMisses:             cfg.MaxMisses,
			MaxMissesConfirmed:    cfg.MaxMissesConfirmed,
			MaxTracks:             cfg.MaxTracks,
		})

	case http.MethodPut:
		var body TrackerConfigRequest
		...
		if body.GatingDistanceSquared != nil {
			api.tracker.Config.GatingDistanceSquared = float32(*body.GatingDistanceSquared)
		}
		if body.ProcessNoisePos != nil {
			api.tracker.Config.ProcessNoisePos = float32(*body.ProcessNoisePos)
		}
		if body.ProcessNoiseVel != nil {
			api.tracker.Config.ProcessNoiseVel = float32(*body.ProcessNoiseVel)
		}
		if body.MeasurementNoise != nil {
			api.tracker.Config.MeasurementNoise = float32(*body.MeasurementNoise)
		}
		if body.OcclusionCovInflation != nil {
			api.tracker.Config.OcclusionCovInflation = float32(*body.OcclusionCovInflation)
		}
		if body.HitsToConfirm != nil {
			api.tracker.Config.HitsToConfirm = *body.HitsToConfirm
		}
		if body.MaxMisses != nil {
			api.tracker.Config.MaxMisses = *body.MaxMisses
		}
		if body.MaxMissesConfirmed != nil {
			api.tracker.Config.MaxMissesConfirmed = *body.MaxMissesConfirmed
		}
	...
	default:
		api.writeJSONError(w, http.StatusMethodNotAllowed, "method not allowed; use GET or PUT")
	}
}
