...
func TestWebServer_HandleExportSnapshotASC_WithSnapshotID(t *testing.T) {
	stats := NewPacketStats()
	config := WebServerConfig{
		Address:           ":0",
		Stats:             stats,
	...
	// Test with snapshot_id parameter - should return not implemented
	req := httptest.NewRequest(http.MethodGet, "/api/lidar/export_snapshot?sensor_id=test-sensor&snapshot_id=123", nil)
	rr := httptest.NewRecorder()
...
func TestWebServer_HandleExportSnapshotASC_NoDB(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/lidar/export_snapshot?sensor_id=test-sensor", nil)
	rr := httptest.NewRecorder()

	server.handleExportSnapshotASC(rr, req)

	// Should return 500 when no DB configured
	if rr.Code != http.StatusInternalServerError {
		t.Errorf("expected 500, got %d: %s", rr.Code, rr.Body.String())
	}
}

func TestWebServer_HandleExportSnapshotASC_WithDB_NoSnapshot(t *testing.T) {
	wrappedDB, cleanup := setupTestDBWrapped(t)
	defer cleanup()
	...
	// Should return 404 when no snapshot exists
	if rr.Code != http.StatusNotFound {
		t.Errorf("expected 404, got %d: %s", rr.Code, rr.Body.String())
	}
}

// ====== handleExportFrameSequenceASC tests ======

func TestWebServer_HandleExportFrameSequenceASC_MissingSensorID(t *testing.T) {
	stats := NewPacketStats()
	config := WebServerConfig{
		Address:           ":0",
		Stats:             stats,
	...
	server.handleExportFrameSequenceASC(rr, req)
...
func TestWebServer_HandleExportFrameSequenceASC_NoFrameBuilder(t *testing.T) {
	stats := NewPacketStats()
	config := WebServerConfig{
		Address:           ":0",
		Stats:             stats,
	...
	req := httptest.NewRequest(http.MethodGet, "/api/lidar/export_frame_sequence?sensor_id=nonexistent-sensor", nil)
	rr := httptest.NewRecorder()

	server.handleExportFrameSequenceASC(rr, req)

	// Should return 404 when no FrameBuilder exists
	if rr.Code != http.StatusNotFound {
		t.Errorf("expected 404, got %d", rr.Code)
	}
}
