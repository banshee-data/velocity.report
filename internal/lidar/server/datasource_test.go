...
// MockServerOps implements ServerDataSourceOperations for testing.
...
func (m *MockServerOps) StartLiveListenerInternal(ctx context.Context) error {
	m.StartLiveCalls++
	return m.StartLiveErr
}

func (m *MockServerOps) StopLiveListenerInternal() {
	m.StopLiveCalls++
}

func (m *MockServerOps) StartPCAPInternal(file string, config ReplayConfig) error {
	m.StartPCAPCalls++
	m.LastPCAPFile = file
...
func (m *MockServerOps) StopPCAPInternal() {
	m.StopPCAPCalls++
}

func (m *MockServerOps) BaseContext() context.Context {
...
func TestRealDataSourceManager_StartLiveListener_Error(t *testing.T) {
	ops := &MockServerOps{StartLiveErr: errors.New("bind failed")}
	mgr := NewRealDataSourceManager(ops)
