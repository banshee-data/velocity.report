...
// newTestServerForDirectBackend creates a minimal Server for direct backend testing.
func newTestServerForDirectBackend(t *testing.T, tracker *l5tracks.Tracker) *Server {
	t.Helper()
	return &Server{
...
func TestNewDirectBackend(t *testing.T) {
	ws := newTestServerForDirectBackend(t, nil)
...
func TestDirectBackend_ResetGrid_WithManager(t *testing.T) {
	...
	ws := &Server{sensorID: sensorID, tracker: tracker}
	db := NewDirectBackend(sensorID, ws)
