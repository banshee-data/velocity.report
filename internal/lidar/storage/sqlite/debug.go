...
// SetLogWriters configures the three logging streams for the sqlite package.
// Pass nil for any writer to disable that stream.
func SetLogWriters(ops, diag, trace io.Writer) {
	opsLogger = logutil.NewTaggedLogger("[sqlite] ", ops)
	diagLogger = logutil.NewTaggedLogger("[sqlite] ", diag)
	traceLogger = logutil.NewTaggedLogger("[sqlite] ", trace)
}
