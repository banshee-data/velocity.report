...
if __name__ == "__main__":
    ...
    @patch("pdf_generator.TimeSeriesChartBuilder")
    @patch("pdf_generator.HistogramChartBuilder")
    @patch("pdf_generator.save_chart_as_pdf")
    def test_tex_file_contains_expected_structure(
        self, mock_save_chart, mock_histogram_builder, mock_timeseries_builder
    ):
        """Test that generated .tex file has expected LaTeX structure."""
        mock_timeseries_builder.return_value.build.return_value = MagicMock()
        mock_histogram_builder.return_value.build.return_value = MagicMock()
        mock_save_chart.return_value = True
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            tex_path = output_path.replace(".pdf", "_report.tex")
            with open(tex_path, "r") as f:
                content = f.read()
            ...
            self.assertIn(r"\usepackage{fontspec}", content)
            self.assertIn(r"\usepackage{graphicx}", content)
            self.assertIn(r"\usepackage{supertabular}", content)
    ...
    def test_tex_file_contains_correct_metrics(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check vehicle count
            self.assertIn("3,469", content)

            # Check key metrics (from overall_metrics)
            self.assertIn("53.52 mph", content)  # Max speed
            self.assertIn("43.05 mph", content)  # p98
            self.assertIn("36.94 mph", content)  # p85
            self.assertIn("30.54 mph", content)  # p50
    ...
    @patch("pdf_generator.HistogramChartBuilder")
    @patch("pdf_generator.save_chart_as_pdf")
    def test_tex_file_contains_date_range(
        self, mock_save_chart, mock_histogram_builder, mock_timeseries_builder
    ):
        """Test that .tex file contains correct date range."""
        mock_timeseries_builder.return_value.build.return_value = MagicMock()
    ...
    @patch("pdf_generator.HistogramChartBuilder")
    @patch("pdf_generator.save_chart_as_pdf")
    def test_tex_file_references_chart_files(
        self, mock_save_chart, mock_histogram_builder, mock_timeseries_builder
    ):
        """Test that .tex file references the generated chart files."""
        mock_timeseries_builder.return_value.build.return_value = MagicMock()
        mock_histogram_builder.return_value.build.return_value = MagicMock()
        mock_save_chart.return_value = True
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check for includegraphics with chart files
            # Note: filename patterns will vary based on prefix
            self.assertIn(r"\includegraphics", content)

            # Should reference histogram and stats charts
            # The exact filenames will have the prefix
            self.assertTrue(
                re.search(r"\\includegraphics.*histogram\.pdf", content)
                or re.search(r"\\includegraphics.*_histogram\.pdf", content)
            )
            self.assertTrue(
                re.search(r"\\includegraphics.*stats\.pdf", content)
                or re.search(r"\\includegraphics.*_stats\.pdf", content)
            )
    ...
    def test_tex_file_contains_histogram_table(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check for histogram table content
            # Should have bucket ranges and counts from our test data
            self.assertIn("5{-}10", content)  # Bucket range
            self.assertIn("66&", content)  # Count for 5-10 bucket (with & separator)
    ...
    def test_tex_file_contains_daily_metrics_table(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check for daily metrics from our test data
            # Should have data from daily_metrics
            self.assertIn("891", content)  # Count from first day
            self.assertIn("1315", content)  # Count from second day
            self.assertIn("1263", content)  # Count from third day
    ...
    @patch("pdf_generator.HistogramChartBuilder")
    @patch("pdf_generator.save_chart_as_pdf")
    def test_chart_builders_called_with_correct_data(
        self, mock_save_chart, mock_histogram_builder, mock_timeseries_builder
    ):
        """Test that chart builders are called with correct data."""
        mock_ts_instance = MagicMock()
        mock_hist_instance = MagicMock()
        mock_timeseries_builder.return_value = mock_ts_instance
        mock_histogram_builder.return_value = mock_hist_instance
        mock_save_chart.return_value = True
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Verify chart builders were instantiated
            mock_timeseries_builder.assert_called()
            mock_histogram_builder.assert_called()

            # Verify build methods were called
            mock_ts_instance.build.assert_called()
            mock_hist_instance.build.assert_called()

            # Verify save_chart_as_pdf was called (twice: histogram + timeseries)
            self.assertEqual(mock_save_chart.call_count, 2)
...
class TestPDFGenerationEdgeCases(unittest.TestCase):
    ...
    @patch("pdf_generator.HistogramChartBuilder")
    @patch("pdf_generator.save_chart_as_pdf")
    def test_generate_with_empty_histogram(
        self, mock_save_chart, mock_histogram_builder, mock_timeseries_builder
    ):
        """Test PDF generation with empty histogram."""
        mock_timeseries_builder.return_value.build.return_value = MagicMock()
