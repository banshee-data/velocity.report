...
func (sudoSystemdActor) DisableTailscaled(ctx context.Context) error {
	return runFixedArgv(ctx, sudoDisableArgv)
}
...
type Manager struct {
	...
	// Per-step results from the most recent applyDevicePolicy run.  Two
	// independent error strings rather than one because SSH and serve
	// fail for different reasons and the operator's next step is
	// different in each case (e.g. ACL fix vs. enabling HTTPS certs).
	sshOK       bool
	sshErr      string
	serveOK     bool
	serveErr    string
...
// fire this once per Enable rather than on every Running transition so
// a network blip after enrolment doesn't generate duplicate writes.
// (Disable() clears the serve config and the in-memory policy results,
// so we own this configuration end-to-end.)
//
// The two steps are reported independently — partial success (e.g. SSH
// up, serve config rejected because HTTPS certs aren't enabled on the
// tailnet) is the common case, and the operator needs to see which
// half failed.
//
...
func (m *Manager) applyDevicePolicy(epoch uint64) {
	...
	sshErr := m.enableSSH(ctx)
	if sshErr != nil {
		log.Printf("tailscale: failed to enable SSH (epoch=%d): %v", epoch, sshErr)
	}
	serveErr := m.enableServeWithRetry(ctx)
	if serveErr != nil {
		log.Printf("tailscale: failed to publish via tailscale serve (epoch=%d): %v", epoch, serveErr)
	}

	m.mu.Lock()
	m.sshOK = sshErr == nil
	if sshErr != nil {
		m.sshErr = sshErr.Error()
	} else {
		m.sshErr = ""
	}
	m.serveOK = serveErr == nil
	if serveErr != nil {
		m.serveErr = serveErr.Error()
	} else {
		m.serveErr = ""
	}
	m.mu.Unlock()
...
type Status struct {
	...
	// SSHEnabled reports whether Tailscale SSH is on.  False when no
	// apply has run yet.
	SSHEnabled bool `json:"ssh_enabled"`
	// SSHError is the most recent error from enabling Tailscale SSH,
	// or "" if the last attempt succeeded or none has run.
	SSHError string `json:"ssh_error,omitempty"`
	// ServePublished reports whether the local web UI is being
	// proxied via `tailscale serve` on :443 of the MagicDNS name.
	ServePublished bool `json:"serve_published"`
	// ServeError is the most recent error from configuring
...
func (m *Manager) Status(ctx context.Context) Status {
	...
	if err != nil {
		...
		// derived state (login_in_progress, login_url, policy
		// outcomes) needs to be consistent or the Tailscale card
		// flickers stale enrolment hints after a disable.
		out.LoginURL = ""
		out.LoginInProgress = false
		out.SSHEnabled = false
		out.SSHError = ""
		out.ServePublished = false
		out.ServeError = ""
		return out
	}
...
//
// We DO clear the `tailscale serve` configuration before stopping the
// daemon.  Reason: we own that config — applyDevicePolicy rewrites it
// on every Enable — so leaving stale serve entries on disk that point
// at an HTTP target the operator may have moved is a footgun.  An
// operator who has manually edited serve config should not toggle
// Tailscale off and back on; the Settings UI is explicit that this
// resets the serve configuration.
//
...
func (m *Manager) Disable(ctx context.Context) error {
	// Best-effort serve teardown before we lose the local API socket.
	// Failure here is non-fatal: the daemon is about to be stopped and
	// masked, so a leftover serve config has no effect until Enable
	// runs applyDevicePolicy and rewrites it from scratch.
	if err := m.lc.SetServeConfig(ctx, &ipn.ServeConfig{}); err != nil {
		log.Printf("tailscale: SetServeConfig(empty) returned %v (continuing)", err)
	}
...
// runFixedArgv invokes a hard-coded argv slice — there is no string
// formatting and no caller-supplied parameter that ends up in argv.
// The slice itself is package-level (sudoEnableArgv / sudoDisableArgv)
// and matches the literal entries in /etc/sudoers.d.
func runFixedArgv(ctx context.Context, argv []string) error {
	if len(argv) == 0 {
		return fmt.Errorf("tailscale: empty argv")
	}
	cmd := exec.CommandContext(ctx, argv[0], argv[1:]...)
	out, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("%s: %v: %s", argv[len(argv)-1], err, string(out))
	}
	return nil
}
