...
// TestSudoArgvIsLiteral pins the exact argv vectors invoked under sudo.
// The matching entries in /etc/sudoers.d/020_velocity-nopasswd are
...
func TestDisableClearsTransientStateAndStopsDaemon(t *testing.T) {
	...
	// Seed state as if a login had completed and policy had run.
	m.mu.Lock()
	m.browseURL = "https://login.tailscale.com/x"
	m.urlSetAt = time.Now()
	m.loginInProg = true
	m.sshOK = true
	m.sshErr = "stale ssh err"
	m.serveOK = true
	m.serveErr = "stale serve err"
	m.enableEpoch = 7
	m.mu.Unlock()
	...
	// Disable must also tear down the serve config so a re-Enable
	// rebuilds it from scratch.  We assert the SetServeConfig call was
	// made with an empty cfg.
	if got := atomic.LoadInt32(&fc.setServeCfgCalls); got != 1 {
		t.Fatalf("Disable should have called SetServeConfig once, got %d", got)
	}
	if fc.setServeConfigArg == nil || fc.setServeConfigArg.Web != nil || fc.setServeConfigArg.TCP != nil {
		t.Fatalf("Disable should have set an empty serve config, got %+v", fc.setServeConfigArg)
	}
	m.mu.RLock()
	...
	m.mu.RUnlock()
	if browseURL != "" || loginInProg || sshOK || sshErr != "" || serveOK || serveErr != "" || enableEpoch != 0 {
		t.Fatalf("Disable did not clear transient state: url=%q inProg=%v sshOK=%v sshErr=%q serveOK=%v serveErr=%q epoch=%d",
			browseURL, loginInProg, sshOK, sshErr, serveOK, serveErr, enableEpoch)
	}
}
...
func TestStatusOnDaemonUnreachable(t *testing.T) {
	...
	m.loginInProg = true
	m.sshOK = true
	m.sshErr = "previous ssh failure"
	m.serveOK = true
	m.serveErr = "previous serve failure"
	m.mu.Unlock()
	...
	if st.LoginURL != "" || st.LoginInProgress ||
		st.SSHEnabled || st.SSHError != "" ||
		st.ServePublished || st.ServeError != "" {
