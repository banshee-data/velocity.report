...
// Keep the module inert on pages that do not mount the homepage hero.
if (canvas) {
  ...
  function sampleCurb(n) {
    for (let i = 0; i < n; i++) {
      ...
      const side = Math.random() < 0.5 ? -1 : 1;
      pushStatic(
        side * (ROAD_HALF_W + 0.05 + Math.random() * 0.1),
  ...
  function sampleBox(
    ...
    for (let i = 0; i < n; i++) {
      ...
      let lx = (Math.random() - 0.5) * sx,
        ly = (Math.random() - 0.5) * sy,
        lz = (Math.random() - 0.5) * sz;
      if (face === 0) lx = sx / 2;
      else if (face === 1) lx = -sx / 2;
      else if (face === 2) ly = sy / 2;
      else if (face === 3) ly = -sy / 2;
      else if (face === 4) lz = sz / 2;
      else lz = -sz / 2;
      pushStatic(
        cx + lx + (Math.random() - 0.5) * jitter,
  ...
  staticGeom.setAttribute(
    "intensity",
    new THREE.BufferAttribute(sIntensity, 1),
  );
  ...
  Object.values(TYPES).forEach((type) => {
    type.colorObj = new THREE.Color(type.color);
  });
  ...
  function sSphere(arr, i, cx, cy, cz, r, n, jitter = 0.02) {
    ...
    for (let k = 0; k < n; k++, i++) {
      ...
      arr[i * 3 + 0] =
        cx + rr * Math.sin(v) * Math.cos(u) + (Math.random() - 0.5) * jitter;
      arr[i * 3 + 1] = cy + rr * Math.cos(v) + (Math.random() - 0.5) * jitter;
      arr[i * 3 + 2] =
        cz + rr * Math.sin(v) * Math.sin(u) + (Math.random() - 0.5) * jitter;
    }
    return i;
  }
  ...
  function localTruck(t) {
    ...
    i = sBoxFace(
      ...
      0,
      t.sy * 0.06,
      (cargoFront + cargoBack) / 2,
      t.sx,
      t.sy * 0.92,
      cargoBack - cargoFront,
      180,
      0.025,
    );
  ...
  function localPed(t) {
    ...
    // Head: small sphere
    i = sSphere(arr, i, 0, headCY, 0, 0.1, 10);
    ...
    while (i < t.density) {
      i = sSphere(arr, i, 0, headCY, 0, 0.1, 1);
    }
    return arr;
  }
  ...
  function localBike(t) {
    ...
    const seatY = wheelY + 0.7; // saddle height
    ...
    i = sSphere(arr, i, 0, torsoEndY + 0.15, torsoEndZ - 0.05, 0.1, 8);
    ...
    while (i < t.density) {
      i = sLine(
        ...
        0,
        bbY,
        (wzFront + wzRear) / 2 + (Math.random() - 0.5) * 0.3,
  ...
  const MAX_TRACKED = 8;
  const MOVING_CAPACITY =
    Math.max(...Object.values(TYPES).map((t) => t.density)) * MAX_TRACKED;
  ...
  movingGeom.setAttribute(
    "intensity",
    new THREE.BufferAttribute(mIntensity, 1),
  );
...
`;
  ...
  function drawLabel(sprite, id, klass, speedMph, conf) {
    ...
    ctx.lineTo(canvas.width - 20, 2);
    ctx.moveTo(2, canvas.height - 18);
    ctx.lineTo(2, canvas.height - 2);
    ctx.lineTo(20, canvas.height - 2);
    ctx.moveTo(canvas.width - 2, canvas.height - 18);
    ...
    ctx.fillText(
      `${klass}  ·  ${speedMph.toFixed(0)} mph  ·  ${(conf * 100).toFixed(0)}%`,
      14,
      40,
    );
  ...
  function makeTrail(color) {
    ...
    const orderedPositions = new Float32Array(TRAIL_LEN * 3);
    const colors = new Float32Array(TRAIL_LEN * 3);
    const positionAttr = new THREE.BufferAttribute(orderedPositions, 3);
  ...
  function attachVisuals(slot) {
    ...
    slot.box = makeCornerBox(t.sx, t.sy, t.sz, t.colorObj);
    scene.add(slot.box);
    ...
    for (let i = 0; i < TRAIL_LEN; i++) {
      ...
      slot.trail.orderedPositions[i * 3 + 0] = slot.pose.x;
      slot.trail.orderedPositions[i * 3 + 1] = 0.05;
      slot.trail.orderedPositions[i * 3 + 2] = slot.pose.z;
      const fade = TRAIL_LEN > 1 ? i / (TRAIL_LEN - 1) : 1;
      slot.trail.colors[i * 3 + 0] = slot.trail.baseColor.r * fade;
      slot.trail.colors[i * 3 + 1] = slot.trail.baseColor.g * fade;
      slot.trail.colors[i * 3 + 2] = slot.trail.baseColor.b * fade;
    }
    slot.trail.positionAttr.needsUpdate = true;
    slot.trail.colorAttr.needsUpdate = true;
  }
  ...
  function spawn(slot) {
    ...
    slot.cruiseSpeed =
      beh.cruiseMin + Math.random() * (beh.cruiseMax - beh.cruiseMin);
    ...
    if (type === "ped") {
      const side = Math.random() < 0.5 ? -1 : 1;
      slot.lane =
        side * (ROAD_HALF_W + WALK_W * 0.5 + (Math.random() - 0.5) * 0.4);
  ...
  function behave(slot, t, dt) {
    ...
    if (beh.followGap > 0) {
      ...
      if (nearestLead) {
        const desiredGap =
          Math.abs(slot.currentSpeed) * beh.followGap + beh.minGap;
    ...
    const wobbleOffset =
      slot.wobbleAmp *
      Math.sin(t * slot.wobbleFreq * 2 * Math.PI + slot.wobblePhase);
  ...
  function tick() {
    ...
    if (spawnCooldown <= 0 && activeCount < 5) {
      ...
      if (free) {
        spawn(free);
        spawnCooldown = 0.6 + Math.random() * 1.4;
      }
    }
    ...
    for (const slot of slots) {
      ...
      slot.conf =
        Math.min(0.92, slot.life * 1.6) + Math.sin(t * 4 + slot.pose.z) * 0.02;

      const t_ = TYPES[slot.type];
      const c = t_.colorObj;
      const cosY = Math.cos(slot.pose.yaw),
        sinY = Math.sin(slot.pose.yaw);

      const local = slot.local;
      const n = slot.bufCount;
      for (let i = 0; i < n; i++) {
        const lx = local[i * 3 + 0],
          ly = local[i * 3 + 1],
          lz = local[i * 3 + 2];
      ...
      if (slot.trail && frameCount % 3 === 0) {
        ...
        for (let i = 0; i < total; i++) {
          ...
          tr.orderedPositions[i * 3 + 0] = tr.positions[idx * 3 + 0];
          tr.orderedPositions[i * 3 + 1] = tr.positions[idx * 3 + 1];
          tr.orderedPositions[i * 3 + 2] = tr.positions[idx * 3 + 2];
          const f = total > 1 ? i / (total - 1) : 1;
          tr.colors[i * 3 + 0] = tr.baseColor.r * f;
          tr.colors[i * 3 + 1] = tr.baseColor.g * f;
          tr.colors[i * 3 + 2] = tr.baseColor.b * f;
        }
        tr.positionAttr.needsUpdate = true;
        tr.colorAttr.needsUpdate = true;
