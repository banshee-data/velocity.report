...
def test_config_order_sync_helpers_and_targets(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="invalid JSON block"):
        mod.parse_markdown_json_blocks(invalid_md.read_text(encoding="utf-8"), invalid_md)
    ...
    assert mod.normalize_source_path("relative.json", tmp_path) == (tmp_path / "relative.json").resolve()
    ...
    reordered, still_missing = mod.reorder_object(existing, canonical, fill_missing=True)
    ...
    blocks = mod.parse_markdown_json_blocks(markdown.read_text(encoding="utf-8"), markdown)
    best = mod.choose_best_markdown_block(blocks, canonical)
    assert list(best.obj.keys()) == ["l3", "version"]

    target_json = tmp_path / "target.json"
    target_json.write_text('{"l3":{"beta":22},"version":1,"extra":true}\n', encoding="utf-8")
    ...
    assert (ok, changed) == (True, True)
    assert msg.startswith("[SYNC]")
    synced_blocks = mod.parse_markdown_json_blocks(markdown.read_text(encoding="utf-8"), markdown)
    assert list(synced_blocks[1].obj["l3"].keys()) == ["alpha", "beta"]


def test_config_order_sync_main_and___main__(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_readme_maths_helpers_and_main(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    mod = load_script_module("readme_maths_check_test", "scripts/readme-maths-check")

    assert mod.is_valid_key("l3.ema_baseline_v1.noise_relative", allow_private=False) is True
    ...
    assert mod.flatten_leaf_paths({"version": 1, "_note": "x", "l3": {"alpha": 1}}) == ["version", "l3.alpha"]
    ...
    readme_path.write_text("```json\n{\"version\":1,\"l3\":{\"alpha\":1}}\n```\n", encoding="utf-8")
    extra_doc = docs_dir / "extra.md"
    extra_doc.write_text("```json\n{\"version\":1,\"l3\":{\"alpha\":1}}\n```\n", encoding="utf-8")
    ...
    (no_tuning_root / "config/README.maths.md").write_text("- `version`\n", encoding="utf-8")
    (no_tuning_root / "config/README.md").write_text("```json\n{\"version\":1}\n```\n", encoding="utf-8")
    ...
    (missing_required_root / "config/README.maths.md").write_text("- `version`\n", encoding="utf-8")
    (missing_required_root / "config/README.md").write_text("```json\n{\"l3\":{\"alpha\":1}}\n```\n", encoding="utf-8")
    (missing_required_root / "config/tuning.defaults.json").write_text('{"version":1}\n', encoding="utf-8")
    ...
    (top_level_list_root / "config/README.maths.md").write_text("- `version`\n", encoding="utf-8")
    (top_level_list_root / "config/README.md").write_text("```json\n{\"version\":1}\n```\n", encoding="utf-8")
    (top_level_list_root / "config/tuning.list.json").write_text("[1, 2]\n", encoding="utf-8")
    monkeypatch.chdir(top_level_list_root)
    monkeypatch.setattr(sys, "argv", ["readme-maths-check", "--tuning-json-glob", "config/tuning.list.json"])
    assert mod.main() == 1

    invalid_json_root = tmp_path / "invalid-json"
    (invalid_json_root / "config").mkdir(parents=True)
    (invalid_json_root / "config/README.maths.md").write_text("- `version`\n", encoding="utf-8")
    (invalid_json_root / "config/README.md").write_text("```json\n{\"version\":1}\n```\n", encoding="utf-8")
    (invalid_json_root / "config/tuning.invalid.json").write_text("{bad}\n", encoding="utf-8")
    monkeypatch.chdir(invalid_json_root)
    monkeypatch.setattr(sys, "argv", ["readme-maths-check", "--tuning-json-glob", "config/tuning.invalid.json"])
    assert mod.main() == 1
