...
spec = importlib.util.spec_from_file_location("jump_candidates", Path(__file__).with_name("lidar-jump-candidates.py"))
...
class JumpCandidatesTest(unittest.TestCase):
    ...
    def test_repeatable_population_and_gap_exclusion(self):
        with sqlite3.connect(":memory:") as conn:
            conn.executescript("CREATE TABLE lidar_tracks(track_id TEXT,sensor_id TEXT,max_speed_mps REAL);"
                               "CREATE TABLE lidar_track_observations(track_id TEXT,ts_unix_nanos INTEGER,x REAL,y REAL);")
            for name, speed, gap in [("jump", 10, 100_000_000), ("slow", 1, 100_000_000), ("gap", 10, 1_000_000_000)]:
                conn.execute("INSERT INTO lidar_tracks VALUES (?,?,?)", (name, "site", speed))
                conn.executemany("INSERT INTO lidar_track_observations VALUES (?,?,?,?)",
            ...
            conn.execute("UPDATE lidar_track_observations SET y=0 WHERE track_id='jump'")
