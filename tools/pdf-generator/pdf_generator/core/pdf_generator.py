    ...
    if site_config_periods:
        ...
        if periods_table:
            ...
            has_unconfigured = any(
                not period.get("variable_config")
                or period.get("variable_config", {}).get("cosine_error_angle") is None
            ...
            if has_unconfigured:
                doc.append(
                    NoEscape(
                        r"\textcolor{red}{\textbf{Warning: Some time periods have no cosine angle configured. Speeds for these periods may be inaccurate.}}"
                    )
                )
                doc.append(NoEscape("\\par"))
