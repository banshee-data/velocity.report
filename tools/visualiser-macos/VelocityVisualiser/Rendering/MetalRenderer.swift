...
class MetalRenderer: NSObject, MTKViewDelegate {
    ...
    private func updatePointBuffer(_ pointCloud: PointCloudFrame) {
        ...
        pointBuffer = device.makeBuffer(bytes: vertices, length: bufferSize, options: .storageModeShared)
        pointCount = count
    }
    ...
    private func updateBoxInstances(_ trackSet: TrackSet) {
        ...
        if !instances.isEmpty {
            ...
            boxInstances = device.makeBuffer(bytes: instances, length: bufferSize, options: .storageModeShared)
    ...
    private func updateTrailBuffer(_ trackSet: TrackSet) {
        ...
        if !vertices.isEmpty {
            let bufferSize = vertices.count * MemoryLayout<Float>.stride
            trailVertices = device.makeBuffer(bytes: vertices, length: bufferSize, options: .storageModeShared)
    ...
    func draw(in view: MTKView) {
        ...
        // Draw boxes
        if showBoxes, let pipeline = boxPipeline, let instances = boxInstances, boxInstanceCount > 0 {
            encoder.setRenderPipelineState(pipeline)
            encoder.setVertexBuffer(instances, offset: 0, index: 0)
            encoder.setVertexBytes(&uniforms, length: MemoryLayout<Uniforms>.stride, index: 1)
            // TODO: Draw instanced boxes with box vertices
        }
        
        // Draw trails
        if showTrails, let pipeline = trailPipeline, let vertices = trailVertices, trailVertexCount > 0 {
            encoder.setRenderPipelineState(pipeline)
...
struct Camera {
    ...
    var projectionMatrix: simd_float4x4 {
        simd_float4x4(perspectiveFov: fov * .pi / 180, aspectRatio: aspectRatio, near: nearPlane, far: farPlane)
    }
}
