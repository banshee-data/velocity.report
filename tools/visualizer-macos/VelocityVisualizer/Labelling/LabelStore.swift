...
class LabelStore {
    ...
    func addLabel(
        trackID: String,
        classLabel: String,
        startFrameID: UInt64? = nil,
        endFrameID: UInt64? = nil,
        annotator: String = "",
        notes: String = ""
    ) -> LabelEvent {
        let label = LabelEvent(
            ...
            startFrameID: startFrameID ?? 0,
            endFrameID: endFrameID ?? 0,
            createdNanos: Int64(Date().timeIntervalSince1970 * 1_000_000_000),
            annotator: annotator,
            notes: notes
        )
    ...
    func exportToFile(_ url: URL) throws {
        guard let data = exportToJSON() else {
