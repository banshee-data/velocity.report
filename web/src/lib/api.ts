...
export interface SweepRequest {
	mode: string; // "multi", "noise", "closeness", "neighbour"
	noise_values?: number[];
	closeness_values?: number[];
	neighbour_values?: number[];
	noise_start?: number;
	noise_end?: number;
	noise_step?: number;
	closeness_start?: number;
	closeness_end?: number;
	closeness_step?: number;
	neighbour_start?: number;
	neighbour_end?: number;
	neighbour_step?: number;
	fixed_noise?: number;
	fixed_closeness?: number;
	fixed_neighbour?: number;
	iterations: number;
	interval: string; // e.g. "2s"
	settle_time: string; // e.g. "5s"
	seed: string; // "true", "false", "toggle"
}
...
export interface ComboResult {
	...
	closeness: number;
	neighbour: number;
	overall_accept_mean: number;
	overall_accept_stddev: number;
	nonzero_cells_mean: number;
	nonzero_cells_stddev: number;
	bucket_means: number[];
	bucket_stddevs: number[];
	buckets: string[];
}
 ...
 */
export async function startSweep(request: SweepRequest): Promise<{ status: string }> {
	const res = await fetch(`${API_BASE}/lidar/sweep/start`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
		body: JSON.stringify(request)
	});
	if (!res.ok) {
		...
		throw new Error(text || `Failed to start sweep: ${res.status}`);
	}
	return res.json();
}

/**
 * Get current sweep status and results
 */
export async function getSweepStatus(): Promise<SweepState> {
	const res = await fetch(`${API_BASE}/lidar/sweep/status`);
 ...
 */
export async function stopSweep(): Promise<{ status: string }> {
	const res = await fetch(`${API_BASE}/lidar/sweep/stop`, {
		method: 'POST'
	});
	if (!res.ok) throw new Error(`Failed to stop sweep: ${res.status}`);
	return res.json();
}
