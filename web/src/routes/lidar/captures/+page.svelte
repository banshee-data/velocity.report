	...
	<header class="mb-4 flex items-start justify-between gap-4">
		<div>
			...
			<p class="text-surface-content/60 mt-1 text-sm">
				What is on the capture volumes, grouped into the sessions their packet clocks form. A replay
	...
	<section class="border-surface-300 bg-surface-100 mb-4 rounded border p-3">
		<div class="flex flex-wrap items-center gap-3">
			...
			{#if activeRoot?.last_scan_state === SCAN_UNREACHABLE}
				<span class="rounded bg-red-100 px-2 py-0.5 text-xs text-red-700">volume unreachable</span>
		...
		<p class="text-surface-content/40 mt-2 text-xs">
			A quick scan lists what is there. Probing reads each new capture in full to learn its packet
			extent, which is what sessions and replay cases are built from — minutes per volume on a first
			pass, and nothing at all when nothing has changed.
		</p>
	...
	{:else if roots.length === 0}
		<div class="border-surface-300 rounded border border-dashed p-8 text-center">
			...
			<p class="text-surface-content/60 mx-auto max-w-lg text-sm">
				...
				They are deliberately not addable from here: the safe-directory boundary that stops replay
				reading arbitrary server paths only holds while the set of readable volumes is fixed outside
				the API.
			</p>
		</div>
	{:else}
						...
						<div class="border-surface-300 bg-surface-100 border-t px-3 py-3">
							...
							<div class="mb-3">
								<MotionStrip
									...
									showBoundaries
								/>
								<div class="text-surface-content/50 mt-1 flex justify-between font-mono text-xs">
									<span>{formatClock(s.start_ns, true)}</span>
									<span>{formatDuration((s.end_ns - s.start_ns) / 1_000_000_000)} continuous</span>
							...
							{#if expandedPeriods.length > 0}
								<div class="mb-3">
									...
									<p class="text-surface-content/40 mt-1 text-xs">
										Selecting a period picks every capture it covers — which for a static stretch is
										...
										>
											...
											<span class="text-surface-content/60 w-16 text-right">
												{f.first_packet_ns && f.last_packet_ns
													? formatDuration((f.last_packet_ns - f.first_packet_ns) / 1_000_000_000)
													: '—'}
											</span>
							...
							{#if selectedFileIds.length > 0}
								<div class="border-primary/40 bg-primary/5 mt-3 rounded border p-3">
									<h3 class="text-surface-content mb-2 text-sm font-medium">
										New replay case from {verdict.files.length} capture{verdict.files.length === 1
											? ''
											: 's'}
									</h3>
